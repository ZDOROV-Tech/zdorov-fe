...
2. **Plan investigation**
   ...
   - Reach for the following helpers instead of inventing bespoke commands:
     - `/sc:research` command (deep web research with parallel execution).
     - `/sc:index-repo` command (repository indexing and context compression).
     - Use Task tool with `subagent_type="sc:deep-research"` for research.
     - Use Task tool with `subagent_type="sc:self-review"` for validation.
